...
class Downloader(database.Database):
	...
	def _icon(self, status = None, stalled = None, small = False):
		if self.mIconPath == None:
			skin = self._setting('interface.theme.icon', raw = True).lower()
			index = skin.find('(')
			if index >= 0: skin = skin[:index-1]
			if skin in ['default', '-', '']:
				self.mIconPath = ''
			else:
				self.mIconPath = os.path.join(xbmcaddon.Addon('script.bubbles.icons').getAddonInfo('path'), 'resources', 'media', 'icons', skin.replace(' ', '').lower())
	...
	def _background(self):
		if self.mSkinPath == None:
			...
			if theme in ['default', '-', '']:
				...
				self.mSkinPath = os.path.join(xbmcaddon.Addon('script.bubbles.skins').getAddonInfo('path'), 'resources', 'media', 'skins', theme)
