...
class Premiumize(Debrid):
	...
	def deleteLaunch(self):
		if tools.Settings.getBoolean('accounts.debrid.premiumize.clear'):
...
class PremiumizeInterface(object):
	...
	def _addLink(self, id, closeDialog = False):
		unknown = 'Unknown'
						...
						while True:
							...
							if (conditionShort or conditionLong) and background:
								...
								if answer:
									...
									self._addAction(id = id, type = type)
									return

							# Sleep
							time.sleep(0.5)

						if closeDialog: self.tProgressDialog.close()
					except:
						pass
...
class RealDebrid(Debrid):
	...
	@classmethod
	def referal(self):
		return tools.Settings.getString('link.realdebrid')
	...
	def _request(self, mode, link, parameters = None, httpTimeout = None, httpData = None, httpHeaders = None, httpAuthenticate = True):
		...
		try:
			...
			try: self.mResult = json.loads(result)
			except: self.mResult = result
	...
	def _accountAuthenticationSettings(self):
		...
		authentication = '' if self.mAuthenticationToken == None else '*************'
	...
	def accountAuthenticationReset(self):
		self._accountAuthenticationClear()
	...
	def select(self, id, files, item = None):
		try:
	...
	# Delete on launch
	def deleteLaunch(self):
		if tools.Settings.getBoolean('accounts.debrid.realdebrid.clear'):
			self.deleteAll(wait = False)
...
class RealDebridInterface(object):
		...
		try:
			if self.mDebrid.accountValid():
				if interface.Dialog.option(title = self.Name, message = 33492):
					self.mDebrid.accountAuthenticationReset()
				raise Exception()
			else:
	...
	def add(self, link, title = None):
		id = self.mDebrid.add(link = link, title = title)
	...
	def _addWaitAction(self, id, seconds = None):
		# Ask to close a background dialog, because there is no cancel button as with the foreground dialog.
	...
	def _addWait(self, id):
		try:
			...
			progressDialog = interface.Dialog.progress(title = title, message = descriptionWaiting, background = background)
			if background: progressDialog.update(int(percentage), interface.Dialog.title(title), descriptionWaiting)
			else: progressDialog.update(int(percentage), interface.Format.fontBold(descriptionWaiting))
			...
			while status == RealDebrid.StatusMagnetConversion or status == RealDebrid.StatusFileSelection or status == RealDebrid.StatusQueued:
				...
				if background and self._addWaitAction(id = id):
					return invalid

				dots += '.'
				if len(dots) > 3: dots = ''
				if background: progressDialog.update(int(percentage), interface.Dialog.title(title), descriptionWaiting + dots)
				else: progressDialog.update(int(percentage), interface.Format.fontBold(descriptionWaiting + dots))
				...
				# Select the largest, so that the direct download link points to the main video file.
				# Otherwise, if all files are selected, RealDebrid will create a rar file in the final link.
				if self.mDebrid.selectLargest(id = id, item = item) == True:
					item = self.mDebrid.item(id = id)
					status = item['status']
					if status == RealDebrid.StatusFinished: # In case of "cached" RealDebrid torrents that are available immediatley.
						percentage = 100
						if background: progressDialog.update(int(percentage), interface.Dialog.title(title), descriptionFinalize)
						else: progressDialog.update(int(percentage), interface.Format.fontBold(descriptionFinalize))
			...
			while True:
				...
				if background and self._addWaitAction(id = id, seconds = seconds):
					return invalid
				...
				if apiCounter == apiInterval:
					...
					if percentageNew >= percentage:
						...
						if background: progressDialog.update(int(percentage), interface.Dialog.title(title), description)
						else: progressDialog.update(int(percentage), description)
