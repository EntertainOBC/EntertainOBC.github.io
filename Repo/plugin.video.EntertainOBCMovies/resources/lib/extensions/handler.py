...
class Handler(object):
	...
	def _initialize(self, type):
		...
		try: type = type['source'].lower()
		except: type = type.lower()
		...
		global HandlerServicesTorrents
		...
		if type == self.TypeTorrent:
			if HandlerServicesTorrents == None:
				HandlerServicesTorrents = []
				if tools.Settings.getBoolean('streaming.torrent.enabled'):
					...
					if premiumize.accountValid() and premiumize.streamingTorrent():
						...
						HandlerServicesTorrents.append(handle)
					...
					self.mServices = HandlerServicesTorrents
			...
			if HandlerServicesHoster == None:
				...
				if tools.Settings.getBoolean('streaming.hoster.enabled'):
					...
					if tools.Settings.getBoolean('streaming.hoster.direct.enabled'):
						handle = HandleDirect()
	...
	def handle(self, link, item, name = None, download = False, popups = False):
		self._initialize(item)
		...
		return service.handle(link = link, item = item, download = download, popups = popups)
...
class Handle(object):
	...
	def handle(self, link, item, download = False, popups = False):
		pass
...
class HandlePremiumize(Handle):
	...
	def handle(self, link, item, download = False, popups = False):
		...
		return debridold.resolver(link, debrid = self.id(), title = title)
